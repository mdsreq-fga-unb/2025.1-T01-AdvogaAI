...
export class DocumentModelsController {
  ...
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  async findAllDocumentModels(
    @Query('limit', new DefaultValuePipe(10), ParseIntPipe) limit: number,
    @Query('offset', new DefaultValuePipe(0), ParseIntPipe) offset: number,
    @UserId() userId: string,
    @Query('search') search?: string,
  ) {
    return await this.documentModelsService.findAllByUserId(
      userId,
      limit,
      offset,
      search,
  ...
  @UseGuards(JwtAuthGuard)
  @Get('generated-document-models')
  @ApiOperation({
    summary: 'Listar documentos geerados com paginação e busca',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  async findAllDocumentsGenerated(
    @Query('limit', new DefaultValuePipe(10), ParseIntPipe) limit: number,
    @Query('offset', new DefaultValuePipe(0), ParseIntPipe) offset: number,
    @UserId() userId: string,
    @Query('search') search?: string,
  ) {
    return await this.documentModelsService.findAllGeneratedByUserId(
      userId,
      limit,
      offset,
      search,
  ...
  async findAllTags() {
    return await this.tagSistemaService.findAll();
  }
  ...
  async updateDocumentModel(
    ...
    return await this.documentModelsService.update(id, updateDto);
  }
  ...
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  @HttpCode(HttpStatus.OK)
  async deleteDocumentModel(@Param('id') id: string, @UserId() userId: string) {
    return await this.documentModelsService.delete(id, userId);
  }

  @UseGuards(JwtAuthGuard)
  @Delete('generated-document-models/:id')
  @ApiOperation({ summary: 'Excluir um documento gerado' })
  @ApiParam({
    name: 'id',
  ...
  async deleteGeneratedDocument(
    ...
    return await this.documentModelsService.deleteGeneratedDocument(id, userId);
  }
